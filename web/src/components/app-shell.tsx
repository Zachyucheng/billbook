...
const legalRoutes = [
  { href: "/legal", labelZh: "法律 / 许可", labelEn: "Legal / License" },
];
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <Link href="/" className="flex items-center gap-2.5">
                    <div className="surface-solid flex h-10 w-10 items-center justify-center rounded-[12px] border border-[color:var(--line)]">
                      <BrandMark className="h-6 w-6" title="Billbook 标识" />
                    </div>
                    <div>
  ...
  const activeWorkspaceRoute = useMemo(
    () =>
      [...workspaceRouteItems]
        ...
        ) ?? workspaceRouteItems[0],
    [pathname],
  );

  useEffect(() => {
    workspaceRouteItems.forEach((item) => {
    ...
    <div className="workspace-shell hide-scrollbar min-h-screen px-2.5 py-2.5 text-foreground md:px-3 md:py-3">
      <div className="mx-auto max-w-[1440px] md:pl-[248px]">
          ...
          <div className="flex h-full flex-col p-3">
            ...
            >
              <div className="flex items-center gap-3">
                <BrandMark className="h-10 w-10 rounded-[14px]" title="Billbook 标识" />
        ...
        <div className="min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  <h1 className="text-lg font-semibold tracking-tight">
                    {activeWorkspaceRoute.shortLabel}
                  </h1>
                </div>
              </div>
          ...
          {errorMessage ? (
            <div className="mt-4 rounded-[22px] border border-[color:var(--warning-soft)] bg-[#fff5ef] px-4 py-3 text-sm text-[color:var(--warning)]">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <span>{errorMessage}</span>
                <button
                  type="button"
                  onClick={clearError}
                  className="ui-chip border border-[color:var(--warning-soft)] bg-white text-[12px] font-semibold"
                >
                  关闭
                </button>
              </div>
